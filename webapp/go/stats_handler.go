...
func getUserStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &userCounts, q, params...); err != nil && !errors.Is(err, sql.ErrNoRows) {
	...
	q, params, err = sqlx.In(`SELECT u.id AS user_id, IFNULL(SUM(l2.tip), 0) AS tip FROM users u
		INNER JOIN livestreams l ON l.user_id = u.id	
		INNER JOIN livecomments l2 ON l2.livestream_id = l.id
		WHERE u.id IN (?) GROUP BY u.id`, userIDs)
	if err := tx.SelectContext(ctx, &userTips, q, params...); err != nil && !errors.Is(err, sql.ErrNoRows) {
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &reactions, q, params...); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count reactions: "+err.Error())
	}
	...
	if err := tx.SelectContext(ctx, &totalTips, q, params...); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count tips: "+err.Error())
	}
